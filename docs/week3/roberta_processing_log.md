...
**Re-run** Re-scored after two data changes: (a) the Reddit window was extended to day −5 for Milton, and Helene set to −4..+1 (Helene formed Sept 22, so Sept 21 has no storm content); (b) the White House duplicate bug was fixed (1 post + 46 comments were double-tagged Helene/Milton). Counts below reflect the corrected data; model, preprocessing, and settings are unchanged from the original run.
...
| reddit_relevant_posts       | 3,418       | 99             | 2.9%        |
| reddit_relevant_comments    | 122,026     | 236            | 0.2%        |
| whitehouse_threads_posts    | 12          | 2              | 16.7%       |
| whitehouse_threads_comments | 2,193       | 11             | 0.5%        |
| **Total**                   | **188,337** | **392**        | **0.2%**    |

Truncation is negligible (0.2% overall) and concentrated in _posts_ (longer text) rather than comments, so the 512-token cap has no material effect on results. The White House posts show a high percentage only because the file is tiny (2 of 12).
...
| reddit_relevant_posts       | 0.312 | 0.628 | 0.060 |
| reddit_relevant_comments    | 0.408 | 0.473 | 0.119 |
| whitehouse_threads_posts    | 0.083 | 0.667 | 0.250 |
| whitehouse_threads_comments | 0.539 | 0.356 | 0.105 |
...
Total wall-clock ≈ 45–50 minutes on the T4, dominated by `reddit_relevant_comments` (122,026 rows, ~38 minutes). All other files combined ran in under 12 minutes.
